...
// When click on each nav-link change active-link class
...
// when click on each a href link with hash
...
scrollLink.forEach((link) => {
  link.addEventListener("click", (e) => {
    ...
    // change active-link of nav-link
...
const slider = (container, wrapper, items, autoPlay = false) => {
  ...
  function setItemWidth() {
    ...
    items.forEach((data) => {
      ...
      // set margin right to 0 for last item
...
// worksFilter is select element
worksFilter.addEventListener("change", (e) => {
  const category = e.target.value;
  filterWorks(category);
});

// wait for worksData to be loaded
